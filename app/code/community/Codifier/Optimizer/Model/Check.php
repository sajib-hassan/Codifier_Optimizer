...
class Codifier_Optimizer_Model_Check
{
    ...
    public function perform($storeId = 1, $apply = false)
    {
        ...
        $this->_localXmlFilename =
            Mage::getBaseDir('design') . DS . 'frontend' . DS .  Mage::getDesign()->getPackageName(). DS .Mage::getDesign()->getTheme('layout') .DS .'layout'.DS.'local.xml';
                                ...
                                if($this->_apply) {
                                    ...
                                    try {
                                        if(!file_exists($newFileName)) {
                                            ...
                                            if($result) {
                                                $this->_recommendations['fileoperations'][] = array('success' =>'File copied to '.$newFileName);
                                            } else {
                                                $this->_recommendations['fileoperations'][] = array('error' =>'Could not copy '.$existing .' to '.$newFileName);
                                            }
                                        }
                                    } catch (Exception $e) {
                                        $this->_recommendations['fileoperations'][] = array('error' =>'Could not copy '.$existing .' to '.$newFileName);
                        ...
                        case 'addItem':
                            ...
                            case 'js':
                                if (isset($this->_defaultItems['js'][(string)$action->name])) {
                                    ...
                                    $this->_recommendations['movetoskin'][$handle][(string)$action->name] = true;
                                    if($this->_apply) {
                                        $newFileName = $this->_skinDefaultDir.DS.'js'.DS.(string)$action->name;
                                        ...
                                        try {
                                            ...
                                            $this->_recommendations['fileoperations'][] = array('error' =>'Could not copy '.$existing .' to '.$newFileName .' '.$e->getMessage());
                                        }
                                    }
                                }
                                break;
    ...
    public function specialHandlingForHandle($handle, $update)
    {
        ...
        if ($handle == 'rss_catalog_tag' || $handle == 'tag_list_index' || $handle == 'tag_product_list' || $handle == 'tag_product_list'
            || $handle == 'tag_customer_index' || $handle == 'tag_customer_view'
        ) {
        ...
        if ($handle == 'sales_recurring_profile_index' || $handle == 'sales_recurring_profile_view__tabs' || $handle == 'sales_recurring_profile_view'
        ...
        if($this->_apply) {
            if(!file_exists($this->_localXmlFilename)) {
                try {
                    ...
                    $this->_recommendations['fileoperations'][] = array('success' =>'Created local.xml file for Theme '.$this->_localXmlFilename);
                } catch (Exception $e) {
                    $this->_recommendations['fileoperations'][] = array('error' => 'Could not create local.xml file for Theme '.$this->_localXmlFilename .' '.$e->getMessage());
                }
