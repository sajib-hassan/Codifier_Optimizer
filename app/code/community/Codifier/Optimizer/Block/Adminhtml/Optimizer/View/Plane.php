...
class Codifier_Optimizer_Block_Adminhtml_Optimizer_View_Plane extends Mage_Adminhtml_Block_Abstract
{
    ...
    {
        $check = Mage::getModel('optimizer/check');
        ...
        $html .= '<p>Theme Optimizer inspected your theme and makes recommendations to take advantage of SpeedsterAdvanced\'s feature to split Javascript into two files: One global file which holds the Javascript needed for every page (think libraries) and one page specific file which is variable for each page.</p>';
        ...
        if (isset($result['fileoperations'])) {
            ...
            foreach ($result['fileoperations'] as $message) {
                $html .= '<li class="'.(key($message)=='success'?'success':'error').'-msg">'.current($message) . '</li>';
            }
            $html .= '</ul>';
        }
